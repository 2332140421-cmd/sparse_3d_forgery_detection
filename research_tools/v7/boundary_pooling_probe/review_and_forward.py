...
def _triplet_display(triplet: Mapping[str, Any]) -> dict[str, Any]:
...
def _model_responses(root: Path, examples_by_org: Mapping[str, Sequence[Mapping[str, Any]]], source_id: str, window_id: str) -> dict[str, Any]:
    ...
    for condition in FORWARD_CONDITIONS:
        ...
        groups = [{"group_id": int(key), "component_index": int(key), "q_mean": float(np.mean(values)), "seed_count": len(values), "aggregation_weight": 1.0 / len(q_by_group), "weighted_contribution": float(np.mean(values) / len(q_by_group))} for key, values in sorted(q_by_group.items())]
        output[condition] = {"status": "AVAILABLE" if seed_scores else "MODEL_MISSING", "window_logit_mean": float(np.mean(seed_scores)) if seed_scores else None, "seed_count": len(seed_scores), "groups": groups, "score_semantics": "classifier logit response; not calibrated probability or localization truth"}
    return output
...
def _frame_records(sequence: Any, model_frame_indices: set[int]) -> list[dict[str, Any]]:
    ...
    for index, frame_index in enumerate(sequence.frame_indices.tolist()):
        ...
        points = [[int(slot), None if not np.isfinite(uv[slot, 0]) else float(uv[slot, 0]), None if not np.isfinite(uv[slot, 1]) else float(uv[slot, 1]), bool(vis[slot]), bool(geo[slot])] for slot in range(sequence.num_tracks)]
...
def _detail(root: Path, row: Mapping[str, Any], examples_by_org: Mapping[str, Sequence[Mapping[str, Any]]], default_case: bool) -> dict[str, Any]:
    ...
    h_triplets = [_triplet_display(t) for t in feature["h_support"].get("triplets", [])]
    b_triplets = [_triplet_display(t) for t in feature["b_support"].get("triplets", [])]
    ...
    if default_case and video_path.is_file() and selected_h is not None and h_triplets:
        ...
        for position, frame_index in enumerate(target_frames):
            path = screenshot_dir / f"{_safe(window_id)}__t{position}.png"
            if not path.is_file(): _make_screenshot(video_path, frame_index, path, h_slots, b_slots, sequence, f"{identity['source_id']} {identity['role']}")
    ...
    return {
        ...
        "model_response": _model_responses(root, examples_by_org, str(identity["source_id"]), window_id),
        ...
        "display_boundaries": ["colors identify local group membership, not real/fake", "local response is classifier logit, not calibrated probability or localization truth", "visible, geometry-valid, triplet-common and model-used remain separate"],
    }
...
function fillSources(){const s=$("source");s.innerHTML=sourceList().map(x=>`<option>${esc(x)}</option>`).join('');if(!s.value&&s.options.length)s.value=s.options[0].value;fillWindow('real');fillWindow('fake');loadBoth()}
...
def build(root: Path) -> dict[str, Any]:
    ...
    (review_root / "index.html").write_text(_html_page(), encoding="utf-8")
