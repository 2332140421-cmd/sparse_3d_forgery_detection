...
def _build_feature_arrays(out: Path, row: dict[str, Any], scales: dict[str, float]) -> dict[str, np.ndarray]:
    ...
    x_full = np.concatenate([rgb, xyz, gv[..., None].astype(np.float32), cov[..., None], motion, mv[..., None].astype(np.float32), hist[..., None].astype(np.float32), (1.0 - gv[..., None].astype(np.float32))], axis=-1)
    x_rgb = np.concatenate([rgb, cov[..., None], motion, mv[..., None].astype(np.float32)], axis=-1)
...
def run_features(out: Path, rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        try:
            ...
            arr = _build_feature_arrays(out, row, scales)
    ...
    write_json_atomic(out / "feature_schema.json", {"version": "v8-feature-v1", "full_dim": 13, "rgb_2d_dim": 7, "shapes": {"x": [FRAME_COUNT, CELL_COUNT, "D"], "component_ids": [FRAME_COUNT, CELL_COUNT], "predecessor": [FRAME_COUNT, CELL_COUNT]}, "mask_semantics": "all non-padding cells retained; geometry validity is state, not row filtering"})
...
def run_train(out: Path, rows: list[dict[str, Any]], device: str = "cuda") -> None:
    ...
    for condition in ["FULL", "RGB_2D"]:
        ...
        for seed in SEEDS:
            ...
            input_dim = 13 if condition == "FULL" else 7
            ...
            records.append({"condition":condition,"seed":seed,"status":"COMPLETE","epoch":100,"parameter_count":parameter_count(model),"train_windows":len(train_rows),"validation_windows":len(val_rows),"elapsed_s":_now()-started,"checkpoint":str(ckpt),"loss_first":history[0],"loss_last":history[-1]})
            completed += 1
...
def run_evaluate(out: Path, rows: list[dict[str, Any]], device: str = "cuda") -> dict[str, Any]:
    ...
    for condition in ["FULL","RGB_2D"]:
        ...
        for seed in SEEDS:
            ...
            model=FullCoverageModel(condition,13 if condition=="FULL" else 7).to(device); model.load_state_dict(rec["model"])
